...
#
#
# ttbar.py - create grid from events with tt->W(lv)b,W(lv)b final states
#
#
...
br_qsqr = array( 'd', [0] )
br_b_px = array( 'd', [0] )
br_bbar_px = array( 'd', [0] )
br_b_py = array( 'd', [0] )
br_bbar_py = array( 'd', [0] )
br_b_pz = array( 'd', [0] )
br_bbar_pz = array( 'd', [0] )
br_b_m = array( 'd', [0] )
br_bbar_m = array( 'd', [0] )
...
otree.Branch( 'bbar_px', br_bbar_px, 'bbar_px/D' )
otree.Branch( 'b_py', br_b_py, 'b_py/D' )
otree.Branch( 'bbar_py', br_bbar_py, 'bbar_py/D' )
otree.Branch( 'b_pz', br_b_pz, 'b_pz/D' )
otree.Branch( 'bbar_pz', br_bbar_pz, 'bbar_pz/D' )
otree.Branch( 'b_m', br_b_m, 'b_m/D' )
otree.Branch( 'bbar_m', br_bbar_m, 'bbar_m/D' )
...
for ievt in xrange( nevents ):
    ...
    (tlv_b, tlv_lm, tlv_vr, tlv_bbar, tlv_lp, tlv_vl,
     pdg_lm, pdg_lp) = parser( itree, lepdecays = lepdecays, version=options.ps.upper(), indices = indexmap[ievt] )

    if None in [ tlv_bbar, tlv_lm, tlv_vr, tlv_b, tlv_lp, tlv_vl ]:
    ...
    br_b_px[0] = tlv_b.Px()
    br_bbar_px[0] = tlv_bbar.Px()
    br_b_py[0] = tlv_b.Py()
    br_bbar_py[0] = tlv_bbar.Py()
    br_b_pz[0] = tlv_b.Pz()
    br_bbar_pz[0] = tlv_bbar.Pz()
    br_b_m[0] = tlv_b.M()
    br_bbar_m[0] = tlv_bbar.M()
        ...
        if itree.jet_antiKt4Truth_n > 0:
            for ij in xrange( itree.jet_antiKt4Truth_n ):
                ...
                if j.Pt() > minjetpt and abs(j.Eta()) < maxjeteta and j.DeltaR( tlv_lm ) > 0.4 and j.DeltaR( tlv_lp ) > 0.4:
                    ...
                    if matchb:
                        if j.DeltaR( tlv_b ) < 0.4 or j.DeltaR( tlv_bbar ) < 0.4:
