...
static const real_t* sortdata = NULL; /* used in the quicksort algorithm */
...
{ const int i1 = *(const int*)a;
  ...
  const real_t term1 = sortdata[i1];
  const real_t term2 = sortdata[i2];
...
static real_t* getrank (int n, real_t data[])
...
static int
makedatamask(int nrows, int ncols, real_t*** pdata, int*** pmask)
{ int i;
  real_t** data;
  int** mask;
  data = malloc(nrows*sizeof(real_t*));
  if(!data) return 0;
  ...
  for (i = 0; i < nrows; i++)
  { data[i] = malloc(ncols*sizeof(real_t));
    if(!data[i]) break;
...
static void
freedatamask(int n, real_t** data, int** mask)
{ int i;
  for (i = 0; i < n; i++)
  { free(mask[i]);
    free(data[i]);
...
static
real_t find_closest_pair(int n, real_t** distmatrix, int* ip, int* jp)
/*
...
{ int i, j;
  real_t temp;
  real_t distance = distmatrix[1][0];
  *ip = 1;
  *jp = 0;
  for (i = 1; i < n; i++)
  { for (j = 0; j < i; j++)
...
static int svd(int m, int n, real_t** u, real_t w[], real_t** vt)
/*
...
{ int i, j, k, i1, k1, l1, its;
  ...
  real_t scale = 0.0;
  real_t anorm = 0.0;
  real_t* rv1 = malloc(n*sizeof(real_t));
  if (!rv1) return -1;
  if (m >= n)
...
int pca(int nrows, int ncolumns, real_t** u, real_t** v, real_t* w)
/*
Purpose
=======
...
v          (input) real_t[n][n], where n = min(nrows, ncolumns)
Not used on input.

w          (input) real_t[n], where n = min(nrows, ncolumns)
Not used on input.
...
*/
{
    ...
    real_t* temp = malloc(ncolumns*sizeof(real_t));
    if (!index || !temp)
    {   if (index) free(index);
        if (temp) free(temp);
        return -1;
...
static
real_t euclid (int n, real_t** data1, real_t** data2, int** mask1, int** mask2,
  const real_t weight[], int index1, int index2, int transpose)
...
data1  (input) real_t array
The data array containing the first vector.

data2  (input) real_t array
...
{ real_t result = 0.;
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      { real_t term = data1[index1][i] - data2[index2][i];
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[i][index1] && mask2[i][index2])
      { real_t term = data1[i][index1] - data2[i][index2];
        result += weight[i]*term*term;
        tweight += weight[i];
      }
    }
...
static
real_t cityblock (int n, real_t** data1, real_t** data2, int** mask1,
  int** mask2, const real_t weight[], int index1, int index2, int transpose)
...
static
real_t correlation (int n, real_t** data1, real_t** data2, int** mask1,
  int** mask2, const real_t weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
{ real_t result = 0.;
  ...
  { int i;
    for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      { real_t term1 = data1[index1][i];
        real_t term2 = data2[index2][i];
        real_t w = weight[i];
        sum1 += w*term1;
        sum2 += w*term2;
  ...
  { int i;
    for (i = 0; i < n; i++)
    { if (mask1[i][index1] && mask2[i][index2])
      { real_t term1 = data1[i][index1];
        real_t term2 = data2[i][index2];
        real_t w = weight[i];
        sum1 += w*term1;
        sum2 += w*term2;
        result += w*term1*term2;
...
static
real_t acorrelation (int n, real_t** data1, real_t** data2, int** mask1,
  int** mask2, const real_t weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
real_t ucorrelation (int n, real_t** data1, real_t** data2, int** mask1,
  int** mask2, const real_t weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
real_t uacorrelation (int n, real_t** data1, real_t** data2, int** mask1,
  int** mask2, const real_t weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
real_t spearman (int n, real_t** data1, real_t** data2, int** mask1,
  int** mask2, const real_t weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
{ int i;
  ...
  real_t* tdata1;
  real_t* tdata2;
  tdata1 = malloc(n*sizeof(real_t));
  if(!tdata1) return 0.0; /* Memory allocation error */
  tdata2 = malloc(n*sizeof(real_t));
  ...
  for (i = 0; i < m; i++)
  { const real_t value1 = rank1[i];
    const real_t value2 = rank2[i];
...
static
real_t kendall (int n, real_t** data1, real_t** data2, int** mask1, int** mask2,
  const real_t weight[], int index1, int index2, int transpose)
/*
Purpose
=======
    ...
    { if (mask1[index1][i] && mask2[index2][i])
      { for (j = 0; j < i; j++)
        { if (mask1[index1][j] && mask2[index2][j])
          { real_t x1 = data1[index1][i];
            real_t x2 = data1[index1][j];
            real_t y1 = data2[index2][i];
            real_t y2 = data2[index2][j];
            if (x1 < x2 && y1 < y2) con++;
            if (x1 > x2 && y1 > y2) con++;
            if (x1 < x2 && y1 > y2) dis++;
            if (x1 > x2 && y1 < y2) dis++;
    ...
    { if (mask1[i][index1] && mask2[i][index2])
      { for (j = 0; j < i; j++)
        { if (mask1[j][index1] && mask2[j][index2])
          { real_t x1 = data1[i][index1];
            real_t x2 = data1[j][index1];
            real_t y1 = data2[i][index2];
            real_t y2 = data2[j][index2];
            if (x1 < x2 && y1 < y2) con++;
            if (x1 > x2 && y1 > y2) con++;
            if (x1 < x2 && y1 > y2) dis++;
            if (x1 > x2 && y1 < y2) dis++;
...
static real_t(*setmetric(char dist)) 
  (int, real_t**, real_t**, int**, int**, const real_t[], int, int, int)
{ switch(dist)
...
A real_t-precison number between 0.0 and 1.0.
============================================================================
*/
{ int z;
  static const int m1 = 2147483563;
  static const int m2 = 2147483399;
  const real_t scale = 1.0/m1;
...
static int binomial(int n, real_t p)
/*
Purpose
=======
...
{ const real_t q = 1 - p;
  ...
  { const real_t s = p/q;
    const real_t a = (n+1)*s;
    real_t r = exp(n*log(q)); /* pow() causes a crash on AIX */
    int x = 0;
    real_t u = uniform();
    while(1)
    { if (u < r) return x;
  ...
  { /* Step 0 */
    const real_t fm = n*p + p;
    const int m = (int) fm;
    const real_t p1 = floor(2.195*sqrt(n*p*q) -4.6*q) + 0.5;
    const real_t xm = m + 0.5;
    const real_t xl = xm - p1;
    const real_t xr = xm + p1;
    const real_t c = 0.134 + 20.5/(15.3+m);
    const real_t a = (fm-xl)/(fm-xl*p);
    const real_t b = (xr-fm)/(xr*q);
    const real_t lambdal = a*(1.0+0.5*a);
    const real_t lambdar = b*(1.0+0.5*b);
    const real_t p2 = p1*(1+2*c);
    const real_t p3 = p2 + c/lambdal;
    const real_t p4 = p3 + c/lambdar;
    while (1)
    { /* Step 1 */
      int y;
      int k;
      ...
      { /* Step 5.2 */
        real_t rho = (k/(n*p*q))*((k*(k/3.0 + 0.625) + 0.1666666666666)/(n*p*q)+0.5);
        real_t t = -k*k/(2*n*p*q);
        real_t A = log(v);
        if (A < t-rho) return y;
...
data       (input) real_t[nrows][ncolumns]
...
cdata      (output) real_t[nclusters][ncolumns] if transpose==0
                    real_t[nrows][nclusters] if transpose==1
...
cache      (input) real_t[nrows] if transpose==0
                   real_t[ncolumns] if transpose==1
...
{ switch(method)
  { case 'm':
    { const int nelements = (transpose==0) ? nrows : ncolumns;
      real_t* cache = malloc(nelements*sizeof(real_t));
      if (!cache) return 0;
...
distmatrix (input) real_t array, ragged
...
errors     (output) real_t[nclusters]
...
static int
kmeans(int nclusters, int nrows, int ncolumns, real_t** data, int** mask,
  real_t weight[], int transpose, int npass, char dist,
  real_t** cdata, int** cmask, int clusterid[], real_t* error,
...
static int
kmedians(int nclusters, int nrows, int ncolumns, real_t** data, int** mask,
  real_t weight[], int transpose, int npass, char dist,
  real_t** cdata, int** cmask, int clusterid[], real_t* error,
  int tclusterid[], int counts[], int mapping[], real_t cache[])
{ int i, j, k;
  const int nelements = (transpose==0) ? nrows : ncolumns;
  const int ndata = (transpose==0) ? ncolumns : nrows;
  int ifound = 1;
...
void kcluster (int nclusters, int nrows, int ncolumns,
  real_t** data, int** mask, real_t weight[], int transpose,
...
void kmedoids (int nclusters, int nelements, real_t** distmatrix,
...
{ int i, j, icluster;
  ...
  *ifound = -1;
  ...
  errors = malloc(nclusters*sizeof(real_t));
  if(!errors)
  { free(saved);
    free(centroids);
    return;
...
real_t** distancematrix (int nrows, int ncolumns, real_t** data,
  int** mask, real_t weights[], char dist, int transpose)
/*
Purpose
=======
...
{ /* First determine the size of the distance matrix */
  ...
  for (i = 1; i < n; i++)
  { matrix[i] = malloc(i*sizeof(real_t));
...
{ int i,j;
  ...
  real_t* result = malloc(nelements*sizeof(real_t));
  if (!result) return NULL;
  memset(result, 0, nelements*sizeof(real_t));

  for (i = 0; i < nelements; i++)
  { result[i] += 1.0;
    for (j = 0; j < i; j++)
    { const real_t distance = metric(ndata, data, data, mask, mask, weights,
                                     i, j, transpose);
      if (distance < cutoff)
      { const real_t dweight = exp(exponent*log(1-distance/cutoff));
...
static
Node* pclcluster (int nrows, int ncolumns, real_t** data, int** mask,
  real_t weight[], real_t** distmatrix, char dist, int transpose)
...
weight     (input) real_t[ncolumns] if transpose==0;
                   real_t[nrows]    if transpose==1
The weights that are used to calculate the distance. The length of this vector
...
{ int i, j;
  ...
  { for (i = 0; i < nelements; i++)
    { memcpy(newdata[i], data[i], ndata*sizeof(real_t));
...
{ const Node* node1 = (const Node*)a;
  ...
  const real_t term1 = node1->distance;
  const real_t term2 = node2->distance;
  if (term1 < term2) return -1;
  if (term1 > term2) return +1;
  return 0;
}
...
static
Node* pslcluster (int nrows, int ncolumns, real_t** data, int** mask,
  real_t weight[], real_t** distmatrix, char dist, int transpose)
...
{ int i, j, k;
  ...
  int* index;
  Node* result;
  temp = malloc(nnodes*sizeof(real_t));
  if(!temp) return NULL;
...
static Node* pmlcluster (int nelements, real_t** distmatrix)
/*
...
static Node* palcluster (int nelements, real_t** distmatrix)
/*
Purpose
=======
...
static
void somworker (int nrows, int ncolumns, real_t** data, int** mask,
  const real_t weights[], int transpose, int nxgrid, int nygrid,
  real_t inittau, real_t*** celldata, int niter, char dist)

{ const int nelements = (transpose==0) ? nrows : ncolumns;
  const int ndata = (transpose==0) ? ncolumns : nrows;
  int i, j;
  real_t* stddata = calloc(nelements,sizeof(real_t));
  int** dummymask;
  int ix, iy;
  int* index;
  int iter;
  /* Maximum radius in which nodes are adjusted */
  real_t maxradius = sqrt(nxgrid*nxgrid+nygrid*nygrid);
  ...
  { int ixbest = 0;
    ...
    if (transpose==0)
    { real_t closest = metric(ndata,data,celldata[ixbest],
        mask,dummymask,weights,iobject,iybest,transpose);
      real_t radius = maxradius * (1. - ((real_t)iter)/((real_t)niter));
      real_t tau = inittau * (1. - ((real_t)iter)/((real_t)niter));
    ...
    { real_t closest;
      real_t** celldatavector = malloc(ndata*sizeof(real_t*));
      real_t radius = maxradius * (1. - ((real_t)iter)/((real_t)niter));
      real_t tau = inittau * (1. - ((real_t)iter)/((real_t)niter));
...
static
void somassign (int nrows, int ncolumns, real_t** data, int** mask,
  const real_t weights[], int transpose, int nxgrid, int nygrid,
  real_t*** celldata, char dist, int clusterid[][2])
/* Collect clusterids */
{ const int ndata = (transpose==0) ? ncolumns : nrows;
  int i,j;
...
void somcluster (int nrows, int ncolumns, real_t** data, int** mask,
  const real_t weight[], int transpose, int nxgrid, int nygrid,
  real_t inittau, int niter, char dist, real_t*** celldata, int clusterid[][2])
/*
...
weights    (input) real_t[ncolumns] if transpose==0;
                   real_t[nrows]    if transpose==1
The weights that are used to calculate the distance. The length of this vector
...
celldata (output) real_t[nxgrid][nygrid][ncolumns] if transpose==0;
                  real_t[nxgrid][nygrid][nrows]    if tranpose==1
...
{ const int nobjects = (transpose==0) ? nrows : ncolumns;
  ...
  if (lcelldata==0)
  { celldata = malloc(nxgrid*nygrid*ndata*sizeof(real_t**));
    for (i = 0; i < nxgrid; i++)
    { celldata[i] = malloc(nygrid*ndata*sizeof(real_t*));
      for (j = 0; j < nygrid; j++)
        celldata[i][j] = malloc(ndata*sizeof(real_t));
    }
  }
...
real_t clusterdistance (int nrows, int ncolumns, real_t** data,
  int** mask, real_t weight[], int n1, int n2, int index1[], int index2[],
...
The routine returns the distance in real_t precision.
  ...
  { case 'a':
    { /* Find the center */
      ...
      { real_t distance;
        ...
        cdata[0] = calloc(ncolumns,sizeof(real_t));
        cdata[1] = calloc(ncolumns,sizeof(real_t));
      ...
      { real_t distance;
        ...
        real_t** cdata = malloc(nrows*sizeof(real_t*));
        int** cmask = malloc(nrows*sizeof(int*));
        for (i = 0; i < nrows; i++)
        { count[i] = calloc(2,sizeof(int));
          cdata[i] = calloc(2,sizeof(real_t));
    ...
    { int i, j, k;
      if (transpose==0)
      { real_t distance;
        real_t* temp = malloc(nrows*sizeof(real_t));
        real_t* cdata[2];
        int* cmask[2];
        for (i = 0; i < 2; i++)
        { cdata[i] = malloc(ncolumns*sizeof(real_t));
      ...
      { real_t distance;
        ...
        int** cmask = malloc(nrows*sizeof(int*));
        for (i = 0; i < nrows; i++)
        { cdata[i] = malloc(2*sizeof(real_t));
          cmask[i] = malloc(2*sizeof(int));
        }
        for (j = 0; j < nrows; j++)
        { int count = 0;
